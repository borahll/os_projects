 ...
 */
void create_named_pipes(char* cssc_pipe_name/*, char* sc_pipe_name*/, pid_t pid) {
    sprintf(cssc_pipe_name, "cs_pipe_%d", pid);
    //sprintf(sc_pipe_name, "sc_pipe_%d", pid);

    if (mkfifo(cssc_pipe_name, 0666) == -1 /*|| mkfifo(sc_pipe_name, 0666) == -1*/) {
...
void connect_to_server(const char* mq_name, const char* cssc_pipe_name/*, const char* sc_pipe_name*/, int wsize) {
printf("%s\n", cssc_pipe_name);
    ...
    sprintf(connection_info, "%s %d", cssc_pipe_name/*, sc_pipe_name*/, wsize);
...
void wait_for_connection_confirmation(const char* cssc_pipe_name) {
    ...
    read(cssc_pipe, confirmation, BUFFER_SIZE);
    printf("%s \n", confirmation);
...
void send_message(const char* cssc_pipe_name, int type, const char* data) {
    int cssc_pipe = open(cssc_pipe_name, O_RDWR);
    ...
    write(cssc_pipe, message, message_len);

    close(cssc_pipe);
}

// Function to receive a message from the server
void receive_message(const char* cssc_pipe_name, char* data) {
    int cssc_pipe = open(cssc_pipe_name, O_RDWR);
    char length_buffer[4];
    read(cssc_pipe, length_buffer, 4);
    int message_len = *(int *)length_buffer;

    // Extract data from the message
    char message[BUFFER_SIZE];
    read(cssc_pipe, message, message_len - 4);
...
void send_quit_request(const char* cssc_pipe_name) {
    send_message(cssc_pipe_name, QUIT_ALL_REQUEST, "");
}
...
int main(int argc, char *argv[]) {
    ...
    char cssc_pipe_name[BUFFER_SIZE];
    //char sc_pipe_name[BUFFER_SIZE];
    // Create named pipes
    create_named_pipes(cssc_pipe_name, getpid());
    // Print paths in client code
// Print the names of the named pipes
printf("Client - cssc_pipe_name: %s\n", cssc_pipe_name);
        fflush(stdout);
    ...
    connect_to_server(mq_name, cssc_pipe_name, /*sc_pipe_name,*/ wsize);

    // Wait for connection confirmation from the server
    wait_for_connection_confirmation(cssc_pipe_name);
    ...
    if (comfile != NULL) {
        ...
        while (fgets(command, BUFFER_SIZE, file) != NULL) {
            ...
            send_message(cssc_pipe_name, COMMAND_LINE, command);

            // Receive and print the result from the server
            char result[BUFFER_SIZE];
            receive_message(cssc_pipe_name, result);
        ...
        while (1) {
            ...
            if (strcmp(command, "quit") == 0 || strcmp(command, "quitall") == 0) {
                if (strcmp(command, "quitall") == 0) {
                    ...
                    send_quit_request(cssc_pipe_name);
                }
